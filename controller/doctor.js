...
exports.getDoctor = async (req, res, next) => {
  try {
    let data = await Doctor.findOne(
      { _id: req.userId },
      { name: 1, availability: 1, appointments: 1 }
    );
    if (!data) {
...
exports.postAvailability = async (req, res, next) => {
  ...
  let response = await Doctor.findByIdAndUpdate(
    { _id: req.userId },
    { availability: { startDate, endDate, startTime, endTime } }
  );
  if (response) {
...
exports.getDoctors = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "doctors data fetched successfully", docData: docData });
  } catch (error) {
    throw error;
  }
};
...
exports.getAppointmentsTime = async (req, res, next) => {
  try {
    ...
    const appointments = await Appointment.find(
      { doctor: req.userId, date: date },
      { time: 1, isCompleted: 1 }
    ).sort({ time: 1 });
